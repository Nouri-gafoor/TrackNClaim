...
class _FoundPageState extends State<FoundPage> {
  ...
  Future<void> _submitFinderReport() async {
    if (_formKey.currentState!.validate() && _selectedDate != null) {
      try {
        ...
        // Save found report
        final docRef =
        await FirebaseFirestore.instance.collection("finder_reports").add({
          "uid": user?.uid,
          "category": _category,
        ...
        // Try to match with lost reports
        final lostReports = await FirebaseFirestore.instance
            .collection("lost_reports")
            .where("category", isEqualTo: _category)
            .where("location", isEqualTo: _location)
            .get();

        for (var report in lostReports.docs) {
          final data = report.data();
          final lostDate = (data["date"] as Timestamp?)?.toDate();

          if (lostDate != null &&
              (_selectedDate!.difference(lostDate)).inDays.abs() <= 2) {
            // Create match with embedded summaries
            await FirebaseFirestore.instance.collection("matches").add({
              "lostReportId": report.id,
              "finderReportId": docRef.id,
              "lostUserId": data["uid"],
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Finder report submitted successfully!")),
        );
        ...
        child: Form(
          ...
          child: ListView(
            children: [
              ...
              ElevatedButton(
                onPressed: _isLoading ? null : _submitFinderReport,
                child: _isLoading
                    ? const CircularProgressIndicator(
                    strokeWidth: 2, color: Colors.white)
                    : const Text("Submit Finder Report"),
              ),
            ],
          ),
        ),
